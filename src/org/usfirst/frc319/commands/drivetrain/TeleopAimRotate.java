 ...
 *
 */
public class TeleopAimRotate extends PIDCommand {
	...
	**/
	...
	//blue bot
	double setpoint = 0;
	static double P = .07;//.025;
	static double I = 0.004;
	static double D = 0.0;
	
	static double rightP = .02;
	static double rightI = .0025;
	static double rightD = 0.0;

	long startTime;
	public int numSamplesOnTarget = 0;

    public TeleopAimRotate() {
    ...
    protected void usePIDOutput(double output) {
    	...
    	if(x++ % 5 == 0){
    		...
    		status.setSetpoint(Robot.driveTrain.getHorizontalOffset());
    	...
    	*/
    	//Robot.driveTrain.setLeftRightSpeeds(output*10, -output*10);
    ...
    protected void initialize() {
    	...
    	//Robot.driveTrain.setModeToVelocity();
    ...
    protected void execute() {
    	...
    	*/
    	...
    	else
    	{
    		...
    		if (Math.abs(controller.getError()) > IZONE){ //maybe not at first but if we need it.
        		controller.setPID(rightP, 0, rightD);
        	}
        	else{
        		controller.setPID(rightP, rightI, rightD);
        	}
